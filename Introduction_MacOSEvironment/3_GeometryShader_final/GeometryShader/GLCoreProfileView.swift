...
class GLCoreProfileView: NSOpenGLView {
    ...
    fileprivate func loadShaders() -> Bool {
        ...
        guard let geometryShaderPath = Bundle.main.path(forResource: "ShaderG", ofType: "vsh") else {
            print("Can not load geometry shader")
            return false
        }
        let geometryShaderCompiled = compileShader(shader: &geometryShader, type: GLenum(GL_GEOMETRY_SHADER), filePath: geometryShaderPath)
