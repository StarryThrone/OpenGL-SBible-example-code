...
@property (atomic, assign) GLint proj_matrixLocation;
@property (atomic, assign) GLint normal_lengthLocation;
...
- (void)drawRect:(NSRect)dirtyRect {
    ...
    NSRect bounds = [self bounds];
    GLKMatrix4 rotateX = GLKMatrix4MakeRotation(GLKMathDegreesToRadians((float)_lifeDuration * 81.0f*0.0f), 1.0f, 0.0f, 0.0f);
    GLKMatrix4 rotateY = GLKMatrix4MakeRotation(GLKMathDegreesToRadians((float)_lifeDuration * 45.0f), 0.0f, 1.0f, 0.0f);
    ...
    glUniformMatrix4fv(_mv_matrixLocation, 1, GL_FALSE, mvMatrix.m);
    ...
    glUniformMatrix4fv(_proj_matrixLocation, 1, GL_FALSE, projectionMatrix.m);
    glUniform1f(_normal_lengthLocation, 0.01f);
...
- (BOOL)loadShaders {
    ...
    _mv_matrixLocation = glGetUniformLocation(_program, "mv_matrix");
    _proj_matrixLocation = glGetUniformLocation(_program, "proj_matrix");
    _normal_lengthLocation = glGetUniformLocation(_program, "normal_length");
    return YES;
}
