...
//  EnvMapSphere
//
//  Created by chenjie on 2019/11/19.
...
class GLCoreProfileView: NSOpenGLView {
    ...
    private var environmentTextures = [GLuint](repeating: 0, count: 3)
    ...
    deinit {
        ...
        glDeleteTextures(3, &self.environmentTextures)
    }
    ...
    override func prepareOpenGL() {
        ...
        // 2. 加载纹理素材
        let textureNames = ["spheremap1.ktx", "spheremap2.ktx", "spheremap3.ktx"]
        for index in 0..<textureNames.count {
            let textureFileName = textureNames[index]
            let textureLoaded = TextureManager.shareManager.loadObject(fileName: textureFileName, toTexture: &self.environmentTextures[index], atIndex: GL_TEXTURE0)
            if !textureLoaded {
                print("Load texture failed with file name: " + textureFileName)
                return
            }
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        glBindTexture(GLenum(GL_TEXTURE_2D), self.environmentTextures[2])
