...
class TextureManager {
    ...
    func loadObject(fileName: String, toTexture texture: inout GLuint, atIndex index: Int32) -> Bool {
        ...
        if target == GLenum(GL_TEXTURE_1D) {
            ...
            glTexImage3D(GLenum(GL_TEXTURE_3D), 0, GLint(fileHeader.glInternalFormat), GLsizei(fileHeader.pixelWidth), GLsizei(fileHeader.pixelHeight), GLsizei(fileHeader.pixelDepth), 0, fileHeader.glFormat, fileHeader.glType, bodyDataAddress)
