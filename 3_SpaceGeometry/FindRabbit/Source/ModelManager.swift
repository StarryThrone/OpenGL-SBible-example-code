...
class ModelManager {
    ...
    func render() {
        ...
        if self.index_buffer != 0 {
            glDrawElementsInstanced(GLenum(GL_TRIANGLES), GLsizei(self.num_indices), GLenum(self.index_type), UnsafeRawPointer(bitPattern: 0), 1)
        } else {
    ...
    func render(instanceCount: UInt) {
        ...
        if self.index_buffer != 0 {
            glDrawElementsInstanced(GLenum(GL_TRIANGLES), GLsizei(self.num_indices), GLenum(self.index_type), UnsafeRawPointer(bitPattern: 0), GLsizei(instanceCount))
        } else {
            let object_decl = self.sub_object.first!
